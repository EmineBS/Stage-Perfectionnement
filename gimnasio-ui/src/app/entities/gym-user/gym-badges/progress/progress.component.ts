...
import { CriteriaService, EntityArrayResponseType, EntityArrayResponseTypeBadgeProgress } from 'app/entities/criteria/service/criteria.service';
...
export class ProgressComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ badge }) => {
      console.log('gym id ' + badge?.gymId);
      this.loadBadgeProgress(badge);
    });
  }

  loadBadgeProgress(badge: any): void {
    this.criteriaService
      .queryProgressByBadge(badge?.gymId, badge?.id)
      .subscribe((res: EntityArrayResponseTypeBadgeProgress) => {
        this.progresss = res?.body || [];
